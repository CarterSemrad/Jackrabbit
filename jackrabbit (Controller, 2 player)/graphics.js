...
float playerLegs2(vec3 p){
    ...
    vec3 hip1 = uhip1p2; //hip right
    vec3 jr1 = ujr1p2; //joint right 1, top knee
    vec3 jr2 = ujr2p2; //joint right 2, ankle joint
    vec3 jr3 = ujr3p2; //joint right 3, foot point

    vec3 hip2 = uhip2p2; //hip right
    vec3 jl1 = ujl1p2; //joint right 1, top knee
    vec3 jl2 = ujl2p2; //joint right 2, ankle joint
    vec3 jl3 = ujl3p2; //joint right 3, foot point
...
float playerBody2(vec3 p){

    float rCone = sdRoundCone(p, vec3(0., 0.1, 0.) + uChestPosp2, vec3(0., -0.1, -0.12), 0.18, 0.1);
    float bottom = p.y - (-0.1);
    float d = max(rCone, -bottom);
    d = rCone;
    return d;
...
float playerJacket2(vec3 p, float face){
    vec3 jacketCenter = vec3(0., -0.1, 0.);
    p -= jacketCenter;
    float lift = uLift;
    vec3 p2 = bend(p.zyx, lift);
...
float player2(vec3 p){
    vec3 p2 = p-playerPos2;

    if(length(p-playerPos2) > 1.2){
        return sdSphere(p2, 1.);
    }
    p2 = rotY(p2, -playerAngle2.x);

    vec3 p3 = rotX(p2, uBodyAnglep2.y);

    float face = playerFace2(p3);
    float body = playerBody2(p3);
    float eyes = playerEyes2(p3);
    float legs = playerLegs2(p2);
    float jacket = playerJacket2(p3, face);
...
if(dist <= MIN_DIST){
    ...
    if(player(p) < 0.005){
        ...
        if(playerFace2(p3) <= 0.005){
            vec3 col1 = 2.*vec3(0.1,0.,0.1);
            ...
            if(p3.x > 0.){
                col = mix(col, stars(dir)+vec3(0.3,0.,0.), abs(noise(10.*p3)));
            }
        ...
        if(playerEyes2(p3) <= 0.005){
            col = vec3(0.8);
        }

        else if(playerBody2(p3) <= 0.005){
            float sternumWidth = 0.02;
            float ribGap = 0.04;
            float ribSize = 0.02;
            vec3 bodyNorm = norm;

            col = stars(dir) + noise(2.*dir)*vec3(0.1,0.,0.1);
            if(abs(p3.x) < sternumWidth){
                col = vec3(.8);
                col = 4.*vec3(0.1,0.,0.1);
                col -= (length(p2.x)/(3.*sternumWidth));
            }
            float ribNum = mod(p3.y+abs(p3.x)/5., ribGap+ribSize);
            if(ribNum < ribSize){
        ...
        else if(playerLegs2(p2) <= 0.005){
            vec3 col1 = 4. * vec3(0.1, 0., 0.1);
            col = vec3(0.);
            col += col1*clamp(dot(norm, normalize(vec3(1.))), 0., 1.);
            col += col1*clamp(dot(norm, normalize(vec3(-0.7, 0.5, -0.6))), 0., 1.);
        }

        else if(playerJacket2(p3, playerFace2(p3)) <= 0.005){
            vec3 col1 = vec3(0.2, 0.1, 0.1);
            vec3 p4 = p3 / vec3(1., 12., 1.);
            vec3 jacketNorm = norm + 0.2*noise2Norm(3.*p4);
            col = vec3(0.);
            col += col1*clamp(dot(norm, normalize(vec3(1.))), 0., 1.);
...
uniform vec3 uhip1p2;// = vec3(0.1, -0.1, -0.12); //hip right
uniform vec3 ujr1p2;// = vec3(0.15, -0.26, 0.05); //joint right 1, top knee
uniform vec3 ujr2p2;// = vec3(0.13, -0.35, -0.15); //joint right 2, ankle joint
uniform vec3 ujr3p2;// = vec3(0.13, -0.46, 0.); //joint right 3, foot point

uniform vec3 uhip2p2;// = vec3(-0.1, -0.1, -0.12); //hip right
uniform vec3 ujl1p2;// = vec3(-0.15, -0.26, 0.05); //joint right 1, top knee
uniform vec3 ujl2p2;// = vec3(-0.13, -0.35, -0.15); //joint right 2, ankle joint
uniform vec3 ujl3p2;// = vec3(-0.13, -0.46, 0.); //joint right 3, foot point

uniform vec3 uChestPosp2;
uniform vec3 uHeadPosp2;
uniform float uLiftp2;
uniform vec3 uBodyAnglep2;
`
);
